...
> 段级注释（义注/复注内容内嵌）的独立设计见 **[`docs/reading-annotations.md`](./docs/reading-annotations.md)**：把下一层注释书内容嵌进段落一起读。注释呈现方式分「行内 / 段后」两档（用户设置，段后为带编号脚注列表、可点角标↔脚注联动），窄屏角标折叠展开、平板竖屏常驻右边栏、平板横屏/大平板横屏双栏联动 + 跨栏高亮；注释正文只嵌译文（不嵌巴利原文）。
