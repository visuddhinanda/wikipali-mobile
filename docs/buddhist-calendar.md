...
> 状态：**v0.3（2026-09-10）**，P1–P4 全部实现；五套历法齐备。
...
| B | **日三时** | 选中日期 + 经纬度（定位，失败则城镇名） | 明相 `aruṇuggamana`（民用曙光 + 蒙气差修正）、日中、日暮 |
| C | **飞行计算** | 航班号 + 日期 | 起降机场与时刻；沿航线可能遇到的曙光 / 日中 / 日暮及其发生点 |
...
- 提供本方案需要的全部原语：
  ...
  - `SearchAltitude(body, observer, direction, startTime, limitDays, altitude)` —— 求太阳过任意高度角的时刻，`altitude = -6` 即民用曙光 / 日暮，`-6.833` 即补过蒙气差的明相（§2.2）；
...
量级参考：曼德勒（22°N）约 3–4 分钟，赤道最小，高纬度显著变大。
...
架构上把它抽成 `FlightProvider` 接口，默认 AeroDataBox，**并始终提供「手填起降机场与时刻」的入口** —— 查不到航班时功能不该整个不可用，而且计算部分完全可离线。API key 走现有的服务器配置机制，不硬编码进包。
...
3. 每点用 `astronomy-engine` 的 `Horizon()` 求太阳高度角 h。
4. 事件判定：
   - **明相 / 日暮**：h 穿越 **−6.833°**（升穿 = 明相，降穿 = 日暮；阈值与 §2.2 一致，机上判日界用的是同一个定义），穿越点用二分法细化到秒；
   - **日中**：h 取局部极大值（对飞行中的观察者，「太阳中天」= 高度角极大，不能用地面公式）；
   - 若无穿越，输出「本航程未遇日暮」这类**明确结论**，不是空状态。
...
```
src/calendar/
  astro.ts            # astronomy-engine 的薄封装：明相(含蒙气差)/日中/日暮/月相/朔望
  lunar/
    astronomical.ts   # §2.3
...
8. **飞行**：构造已知案例（如 12 月的赫尔辛基 → 曼谷，途中必遇日暮）做端到端断言。
...
| 期 | 状态 | 说明 |
|---|---|---|
| P1 | ✅ | `astronomy-engine` + 明相/日中/日暮 + 定位与城镇降级（城镇表 34135 条 / 2.0 MB）。**真机 GPS 已验**：Badulla 6.696/81.055 → Asia/Colombo → 明相 05:32 / 日中 12:02 / 日暮 18:32，蒙气差 3′24″ |
| P2 | ✅ 五套 | 天文、缅甸（mmcal 移植）、斯里兰卡、中国农历、泰国（pythaidate 移植） |
| — | ✅ | 国际卫塞节（§2.8）跨历法标识 |
| P4 | ✅ | 大圆插值 + 沿途太阳事件 + 手填起降；机场表 4008 条 / 160 KB。**AeroDataBox 需要自备 key**（`setFlightApiKey`），没 key 时只能手填 |
