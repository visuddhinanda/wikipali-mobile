...
根本正文句子（Sentence 表，content 含 {{sent|id=…}} 模板）
   ...
   └─ 返回给 App（tipitaka-read-para 的 display 字段）
```
...
App 用 `TipitakaReadParaController`（`GET /api/v3/tipitaka-read-para`）拿 HTML；服务端在**渲染句子**时注入注释，流程：
...
- **正文获取复用**：注释内容走 `docs/reading-content.md` 的同一条链路（`tipitaka-read-para` + `para_html` 缓存），App 侧无需新接口；注释记录存在 mint 的 `discussions` 表（§3）。
