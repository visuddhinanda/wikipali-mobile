...
- **`tipitaka-read-para` 不能删**：章节接口的游标会顺延，问某一段有没有它回的
  ...
  写回规则与阅读一致，两条路共用 `para_html`。

### 11.5 章节标题优先用译文

目录库的 `toc` 是巴利文，但标题行本身也是段落，译者通常连标题一起译了 ——
`para_html` 里就有。标题栏、正文大标题、目录抽屉一律「译出了用译文、没译才退回
巴利」，逐条回退（`src/reading/heading.ts` 的 `channelHeadingTexts`）。

坑：译出的标题**不能过巴利转写器**。正文的字体转换按 `class='original'` 判断，
标题是纯文本没这个标记，所以 `ReaderDoc.titlePali` 记一笔，只有巴利那支才转。
...
断点续传不需要状态机，由数据本身推出：查该书已解析且未过期的段，缺口在哪就
从哪续。进度的分子分母见 11.4。
